...
// [[Rcpp::export]]
List MvMqCavLoop(NumericVector c, NumericMatrix a, NumericMatrix r, NumericMatrix y, NumericVector Theta, NumericVector EmpiricalQuantile) {
  int i,j;
  ...
  for (i = 1; i < n; ++i) {
    q(i,_) = c+mmult(q(i-1,_),a,p)+mmult(y(i-1,_),r,p);
  }
